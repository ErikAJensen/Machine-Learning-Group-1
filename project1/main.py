...
def main():
    ...
    print_metrics("CART with ADASYN Hyperparameter Tuned", y_test, cart_ht.predict(X_test), cart_ht.predict_proba(X_test)[:, 1])
    ...
    print_metrics("Random Forest with ADASYN Hyperparameter Tuned", y_test, rf_ht.predict(X_test), rf_ht.predict_proba(X_test)[:, 1])
